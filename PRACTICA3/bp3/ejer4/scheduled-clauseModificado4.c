...
main(int argc,char**argv)
{
  ...
  #pragma omp single   //Solo realizará lo siguiente la hebra que llegue antes a esta clausula. 
  						//Otra opcion seria poner un identificador cualquiera y que unicamente esa hebra entre en el if que contenga el codigo siguiente
	...
	printf("\tNumero de hebras: omp_get_num_threads() = %d\n", omp_get_num_threads());
	printf("\tNumero de procesos: omp_get_num_procs() = %d\n", omp_get_num_procs());
	_Bool x = omp_in_parallel();
	printf("\tEsta en region paralela?: omp_in_parallel() = "); fputs(x ? "true" : "false", stdout);
	printf("\n");
